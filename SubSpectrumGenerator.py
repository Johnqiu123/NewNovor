...
class SubSpectrumGenerator(object):
    ...
    def generateSpecialSubSpectra(self,spectra,bin_len=0.1, bin_area=50, flag = 'num', stype='dual'):
        """
           ...
           assume each ion is b-ion, compute fragmentation site's mass
           b and not b
        ...
        for spectrum in spectra:
            ...
            for i in range(len(masses)-1):
                nmass,cmass = 0,0
                if stype == 'dual':
                    nmass = pep.getMass()- masses[i] + 19 - 1  # cmass + H2O
                elif stype == 'atype':
                    nmass = masses[i]-27 - 1 #masses[i]-27 masses[i]
                elif stype == 'yNH3':
                    nmass = pep.getMass()- masses[i] + 19-17 -1 
                elif stype == 'yH2O':
                    nmass = pep.getMass()- masses[i] +19-18 -1 
                elif stype == 'bH2O':
                    nmass = masses[i]-17 - 1
                elif stype == 'bNH3':
                    nmass = masses[i]-16 - 1
                elif stype == 'y46-':
                    nmass = pep.getMass()- masses[i] + 19-46 -1
                elif stype == 'y45-':
                    nmass = pep.getMass()- masses[i] + 19-45 -1
                elif stype == 'y10+':
                    nmass = pep.getMass()- masses[i] + 19 + 10 - 1
